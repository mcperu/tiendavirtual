<?php
class Categoriatres extends Controllers{
    ...
    public function Categoriatres(){
       
       $data['page_id'] = 6;
       $data['page_tag'] = "Subcategorias Usuario";
       $data['page_name'] = "categoriatres";
       $data['page_title'] = "Subcategorias <small>Tienda Virtual</small>";
	   $data['page_functions_js'] = "functions_categoriatres.js";
      
       $this->views->getView($this,"categoriatres",$data);
    }
		...
		public function getCategoriatres()
		{
			$arrData = $this->model->selectCategoriatres();
		...
		public function getSelectCategoriatres()
		{
			$htmlOptions = "";
			$arrData = $this->model->selectCategoriatres();
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
					if($arrData[$i]['status'] == 1 ){
					$htmlOptions .= '<option value="'.$arrData[$i]['idcategoriatres'].'">'.$arrData[$i]['nombrecategoriatres'].'</option>';
					}
				}
			}
			echo $htmlOptions;
