<?php
class Categoriados extends Controllers{
    ...
    public function Categoriados(){
       
       $data['page_id'] = 6;
       $data['page_tag'] = "Categorias";
       $data['page_name'] = "categoriados";
       $data['page_title'] = "Categorias <small>Tienda Virtual</small>";
	   $data['page_functions_js'] = "functions_categoriados.js";
      
       $this->views->getView($this,"categoriados",$data);
    }
			...
			if($_POST){
				...
				if(empty($_POST['txtNombre']) || empty($_POST['listCategoriaunoid']))
				{
					...
					if($intIdcategoriados == 0){
						//crear Categoriauno
						$opion = 1;
						$request_categoriados = $this->model->insertCategoriados(ucfirst(strtolower($strNombre)),$intCategoriaunoid, $intStatus);
					}else{
						//actualizar Categoriauno
						$opion = 2;
						$request_categoriados = $this->model->updateCategoriados($intIdcategoriados, ucfirst(strtolower($strNombre)), $intCategoriaunoid, $intStatus);
					}
		...
		public function getCategoriados()
		{
			...
			for ($i=0; $i < count($arrData); $i++) {
				...
				<button class="btn btn-primary btn-sm btnEditCategoriados" onClick="fntEditCategoriadosOnly('.$arrData[$i]['idcategoriados'].')" title="Editar"><i class="fas fa-pencil-alt"></i></button>
		...
		public function getSelectCategoriados()
		{
			...
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
					if($arrData[$i]['status'] == 1 ){
					$htmlOptions .= '<option value="'.$arrData[$i]['idcategoriados'].'">'.$arrData[$i]['nombrecategoriados'].'</option>';
					}
				}
			}
			echo $htmlOptions;
